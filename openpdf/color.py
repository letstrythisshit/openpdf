"""Color helpers and PDF color-space utilities.

Colors in OpenPDF are represented as:
  - float (gray): 0.0 = black, 1.0 = white
  - 3-tuple of floats (RGB): each component in [0, 1]
  - 4-tuple of floats (CMYK): each component in [0, 1]

This matches the fitz convention. All conversion functions normalize to
...
ColorLike = Union[float, tuple, list, None]


def normalize_color(color: ColorLike) -> tuple | None:
    """Normalize a color value to a float tuple or None.

    Accepts:
      - None → None (no color / transparent)
      - float → (gray,)  i.e. single-channel gray
      - int → treated as float, normalized to 0-1
      - (r, g, b) → (r, g, b)  — values must be in [0, 1]
      - (c, m, y, k) → (c, m, y, k)
      - Sequence of 1, 3, or 4 elements
    """
    if color is None:
        return None
    ...
    if isinstance(color, (list, tuple)):
        ...
        if n == 1:
            ...
            raise ValueError(f"Color tuple must have 1, 3, or 4 components, got {n}")
    raise TypeError(f"Unsupported color type: {type(color)}")


def color_to_pdf_ops(color: tuple | None, stroke: bool = True) -> str:
    """Convert normalized color tuple to PDF color-setting operators.

    stroke=True  → use stroke operators (G, RG, K)
    stroke=False → use fill operators (g, rg, k)
    """
    if color is None:
        return ""
    n = len(color)
    fmt = " ".join(f"{v:.4f}" for v in color)
    if n == 1:
...
def get_color(name: str) -> tuple:
    """Look up a color by name, returning (r, g, b) in [0, 1] range.

    Supports CSS/SVG color names and the extended set fitz provides.
    """
...
# ---------------------------------------------------------------------------
# Color name lookup table — a representative subset of the ~500 fitz colors.
# Values are (r, g, b) in [0, 1].
# ---------------------------------------------------------------------------

_COLOR_TABLE: dict[str, tuple] = {
